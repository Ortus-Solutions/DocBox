...
/**
 * Pulls the pre-rendered command markup out of an already-generated command HTML
 * page so it can be embedded directly in data/content.js, which avoids the SPA having to 
 * fetch() each command page on demand. This is necessary because browsers block 
 * fetch()/XHR requests made from a file:// page.
 * 
 * @filePath Absolute path to the already-written command HTML page (see class.cfm).
 * @return   The inner HTML of the page's #command-content# container, or "" if the
 *           file or the container markers can't be found.
 */
local.extractCommandContent = function( required string filePath ) {
	if ( !fileExists( arguments.filePath ) ) {
		return "";
	}

	var html        = fileRead( arguments.filePath );
	// These markers must stay in sync with the container written by class.cfm.
	var startMarker = 'id="command-content">';
	var endMarker   = '</div><!-- end container-fluid -->';

	var startPos = findNoCase( startMarker, html );
	if ( startPos == 0 ) {
		return "";
	}
	startPos += len( startMarker );

	var endPos = findNoCase( endMarker, html, startPos );
	if ( endPos == 0 ) {
		return "";
	}

	return mid( html, startPos, endPos - startPos );
};
...
local.namespaceTree = {};
local.topLevelTree  = {};
// Pre-rendered command content, keyed by the same "link" used in navData, so the
// SPA can look content up directly instead of fetch()-ing each command page.
local.contentMap = {};
...
for ( local.row in navArgs.qMetaData ) {
	...
	// Command pages are written to disk before this template runs (see packagePages.cfm),
	// so the content is available to read back and embed for the SPA.
	local.contentMap[ local.link ] = local.extractCommandContent( navArgs.outputDir & "/" & local.link );
...
// ── Write data/content.js ──────────────────────────────────────────────────────
// Embeds every command's pre-rendered HTML so the SPA can render it without a
// fetch() call — see extractCommandContent() above for why that matters.

local.contentJsonContent = serializeJSON( local.contentMap, "struct" );
local.contentJsContent   = "// CommandBox Documentation - Command Content" & chr( 10 )
	& "// Auto-generated by DocBox - Do not edit manually" & chr( 10 )
	& "window.COMMANDBOX_CONTENT_DATA = " & local.contentJsonContent & ";";

fileWrite( local.dataDir & "/content.js", local.contentJsContent );
